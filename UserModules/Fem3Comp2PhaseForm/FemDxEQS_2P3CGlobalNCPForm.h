...
void TemplateTransientDxFEMFunction_2P3CGlobalNCPForm<T_DIS_SYS, T_LINEAR_SOLVER, T_FUNCTION_DATA, T_LOCAL_JACOBIAN_ASSEMBLER>::GlobalJacobianAssembler(T_FUNCTION_DATA* _function_data, LinearSolverType & eqsJacobian_global)
{
	...
	for (size_t node_idx_row = 0; node_idx_row < nnodes; node_idx_row++){
		for (size_t node_idx_col = 0; node_idx_col < 5*nnodes; node_idx_col++){
			double const tmp1 =
			    mat_Jacobian.coeff(3 * nnodes + node_idx_row, node_idx_col);
			double const tmp2=
				mat_Jacobian.coeff(4 * nnodes + node_idx_row, node_idx_col);
			eqsJacobian_global.addA(5*node_idx_row +3, node_idx_col, -tmp1);
			eqsJacobian_global.addA(5 * node_idx_row + 4, node_idx_col, -tmp2);
		}
	}
}
