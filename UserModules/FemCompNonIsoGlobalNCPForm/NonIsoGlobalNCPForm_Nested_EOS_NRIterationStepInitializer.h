...
public:
	...
	void post_process(const T_X &/*dx*/, const T_X & x_new, F_RESIDUALS &/*f_residuals*/, F_DX &/*f_dx*/)
	{
        ...
        for (size_t num_p = 0; num_p < x_new.size()-1; num_p+=4)
		{

			_assemblerJ->get_function_data()->set_P_node_values(num_p/4, x_new[num_p]);
			_assemblerJ->get_function_data()->set_X_node_values(num_p/4, x_new[num_p+1]);
			_assemblerJ->get_function_data()->set_S_node_values(num_p / 4, x_new[num_p + 2]);
			_assemblerJ->get_function_data()->set_T_node_values(num_p / 4, x_new[num_p + 3]);
