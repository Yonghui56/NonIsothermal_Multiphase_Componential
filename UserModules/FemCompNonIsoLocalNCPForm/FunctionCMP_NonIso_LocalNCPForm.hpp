...
bool FunctionCMP_NonIso_LocalNCPForm<T1, T2>::initialize(const BaseLib::Options &option)
{
	...
	_problem = new MyCMPLocalNCPFormProblemType(dis);
	_problem->setTimeSteppingFunction(*tim);  // applying the time stepping function
	
	// creating mean pressure vector
	MyVariableCMPLocalNCPForm* mean_pressure = _problem->addVariable("MEAN_PRESSURE");
	...
	MyVariableCMPLocalNCPForm* total_mass_density = _problem->addVariable("TOTAL_MASS_DENSITY");
	...
	MyVariableCMPLocalNCPForm* temperature = _problem->addVariable("TEMPERATURE");
	...
	MathLib::LocalVector tmpS = MathLib::LocalVector::Ones(1);
	MathLib::LocalVector tmprho_l_h = MathLib::LocalVector::Ones(1);
	MathLib::LocalVector tmp0 = MathLib::LocalVector::Zero(1); 
	MathLib::LocalVector tmprho_g_h = MathLib::LocalVector::Ones(1);
	tmpS = tmpS*0.5;
	tmprho_l_h = 0.20991*tmprho_l_h;
	tmprho_g_h = 0.8461*tmprho_g_h;
	for (size_t ele_id = 0; ele_id < n_ele; ele_id++)
	{
		...
		for (size_t jj = 0; jj < 4; jj++)
		{
			...
			_S->setIntegrationPointValue(ele_id, jj, tmpS);
			_PG -> setIntegrationPointValue(ele_id, jj, tmp0);
			_PC->setIntegrationPointValue(ele_id, jj, tmp0);
			_rho_G_h->setIntegrationPointValue(ele_id, jj, tmprho_g_h);
			_rho_G_w->setIntegrationPointValue(ele_id, jj, tmp0);
			_rho_L_h->setIntegrationPointValue(ele_id, jj, tmprho_l_h);
		}
	}
	...
	*/
	...
	_non_linear_problem = new MyNonLinearCMPLocalNCPFormProblemType(dis);
	...
	// set up solution
	_solution = new MyCMPLocalNCPFormSolution(dis, _problem, _non_linear_solution, this);
	/**
	...
	*/
	...
	OutputVariableInfo var_Sec_5(this->getOutputParameterName(6), _msh_id, OutputVariableInfo::Element, OutputVariableInfo::Real, 1, _rho_G_h);
	femData->outController.setOutput(var_Sec_5.name, var_Sec_5);
	
	OutputVariableInfo var_Sec_6(this->getOutputParameterName(7), _msh_id, OutputVariableInfo::Element, OutputVariableInfo::Real, 1, _rho_G_w);
	femData->outController.setOutput(var_Sec_6.name, var_Sec_6);
	return true;
}

template <class T1, class T2>
void FunctionCMP_NonIso_LocalNCPForm<T1, T2>::initializeTimeStep(const NumLib::TimeStep &/*time*/)
{
	/*
	size_t i;
...
template <class T1, class T2>
void FunctionCMP_NonIso_LocalNCPForm<T1, T2>::updateOutputParameter(const NumLib::TimeStep &/*time*/)
{
...
template <class T1, class T2>
void FunctionCMP_NonIso_LocalNCPForm<T1, T2>::calc_nodal_eos_sys(double dt = 0.0)
{
