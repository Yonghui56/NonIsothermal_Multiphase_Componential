...
#ifndef NON_LINEAR_CMP_NONISO_TOTALDENSITYFORM_JACOBIAN_LOCAL_ASSEMBLER_H
#define NON_LINEAR_CMP_NONISO_TOTALDENSITYFORM_JACOBIAN_LOCAL_ASSEMBLER_H
...
#include "NonLinearCMP_NonIso_TotalDensityForm_TimeODELocalAssembler.h"
...
class NonLinearCMP_NonIso_TotalDensityForm_JacobianLocalAssembler : public NumLib::IElementWiseTransientJacobianLocalAssembler
{
public:
	typedef MathLib::LocalVector LocalVectorType;
	typedef MathLib::LocalMatrix LocalMatrixType;
	NonLinearCMP_NonIso_TotalDensityForm_JacobianLocalAssembler(FemLib::LagrangeFeObjectContainer* feObjects, T_FUNCTION_DATA* func_data, const MeshLib::CoordinateSystem &problem_coordinates)
	...
	virtual ~NonLinearCMP_NonIso_TotalDensityForm_JacobianLocalAssembler()
    {
		_function_data = NULL; 
	...
	virtual void assemble_jac_ODE(const NumLib::TimeStep & /*time*/, const MeshLib::IElement &e, const MathLib::LocalVector & u1, const MathLib::LocalVector & u0, MathLib::LocalMatrix  & localM, MathLib::LocalMatrix & localK, MathLib::LocalMatrix & localF)
	{
		...
		*/
		...
		for (j = 0; j < n_gsp; j++)
		{
			...
			M(0, 1) = poro;
			M(0, 2) = 0.0;
			M(1, 0) = -poro*(rho_l_std - rho_G_w_gp(0))*dSgdP_gp(0)*Charact_func + poro* S_gp(0)*drho_G_w_dP_gp(0);
			M(1, 1) = poro*(1 - Charact_func*dSgdX_gp(0)*(rho_l_std - rho_G_w_gp(0)) + S_gp(0)*drho_G_w_dX_gp(0));
			M(1, 2) = poro*(rho_G_w_gp(0) - rho_L_std)*dSgdT_gp(0)*Charact_func + poro*S_gp(0)*drho_G_w_dT_gp(0)*Charact_func;

			M(2, 0) = delta_h_vap*(poro*rho_L_std*dSgdP_gp(0)*Charact_func);
			M(2, 1) = delta_h_vap*(poro*rho_L_std*dSgdX_gp(0)*Charact_func);
			M(2, 2) = (1 - poro)*rho_S_std*C_solid + poro*(RHO_G*S_gp(0)*C_pg + RHO_L*(1 - S_gp(0))*C_pl) + delta_h_vap*(poro*rho_L_std*dSgdT_gp(0)*Charact_func);
...
private:
	...
	MathLib::LocalVector PG_h_gp;
	MathLib::LocalVector PG_w_gp;
	...
	const double delta_h_vap =  2258000.0;
	double rho_S_std;
	double gamma_gp;
	std::size_t isinf;
