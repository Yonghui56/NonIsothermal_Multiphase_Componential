...
ProcessType convertProcessType ( const std::string& pcs_type_string )
{
	...
	if (pcs_type_string.compare("CMP_NonIso_TotalDensityForm") == 0)
		return CMP_NonIso_TotalDensityForm;
	if (pcs_type_string.compare("CMP_NonIso_CapPressureForm") == 0)
		return CMP_NonIso_CapPressureForm;
	if (pcs_type_string.compare("CMP_NonIso_GlobalNCPForm") == 0)
		return CMP_NonIso_GlobalNCPForm;
	if (pcs_type_string.compare("CMP_NonIso_LocalNCPForm") == 0)
		return CMP_NonIso_LocalNCPForm;
	if (pcs_type_string.compare("CMP_2P3CGlobalNCPForm") == 0)
		return CMP_2P3CGlobalNCPForm;
	if (pcs_type_string.compare("CMP_TotalMassEXPForm") == 0)
		return CMP_TotalMassEXPForm;
	if (pcs_type_string.compare("CMP_NonIsoTotalMassEXPForm") == 0)
		return CMP_NonIsoTotalMassEXPForm;
    //else
...
std::string convertProcessTypeToString ( ProcessType pcs_type )
{
	...
	if (pcs_type == CMP_TotalDensityForm)
		return "CMP_TotalDensityForm";
	if (pcs_type == CMP_NonIso_TotalDensityForm)
		return "CMP_NonIso_TotalDensityForm";
	if (pcs_type == CMP_NonIso_CapPressureForm)
		return "CMP_NonIso_CapPressureForm";
	if (pcs_type == CMP_NonIso_GlobalNCPForm)
		return "CMP_NonIso_GlobalNCPForm";
	if (pcs_type == CMP_NonIso_LocalNCPForm)
		return "CMP_NonIso_LocalNCPForm";
	if (pcs_type == CMP_2P3CGlobalNCPForm)
		return "CMP_2P3CGlobalNCPForm";
	if (pcs_type == CMP_TotalMassEXPForm)
		return "CMP_TotalMassEXPForm";
	if (pcs_type == CMP_NonIsoTotalMassEXPForm)
		return "CMP_NonIsoTotalMassEXPForm";
    if (pcs_type ==   NO_PCS)
        return "NO_PCS";
